...
public static class RectangleExtensions
{
    ...
    public static Rectangle Multiply(this Rectangle rect, int number)
        => new(rect.X * number, rect.Y * number, rect.Width * number, rect.Height * number);

    /// <summary>
    /// Divides all the values inside <paramref name="rect"/> by the given <paramref name="number"/>.
    /// </summary>
    /// <returns>The rectangle with adjusted location and size</returns>
    public static Rectangle Divide(this Rectangle rect, int number)
        => new(rect.X / number, rect.Y / number, rect.Width / number, rect.Height / number);
    ...
    /// <returns>The rectangle with adjusted location and size</returns>
    public static Rectangle DivideRounding(this Rectangle rect, float value)
        => rect.DivideRounding(value, value);
    ...
    public static Rectangle AddToSize(this Rectangle rect, Point amount)
        => new(rect.Location, rect.Size + amount);

    /// <summary>
    /// Adjusts the size of the <paramref name="rect"/> by the given <paramref name="amount"/>.
    /// </summary>
    /// <returns>The new <see cref="Rectangle"/></returns>
    public static Rectangle AddToSize(this Rectangle rect, int amount)
        => rect.AddToSize(new Point(amount));
    ...
    /// </summary>
    public static Rectangle Translate(this Rectangle rect, Point p)
        => new(rect.Location + p, rect.Size);

    /// <summary>
    /// Translates the rectangle by <paramref name="nb"/>, on both axes
    /// </summary>
    public static Rectangle Translate(this Rectangle rect, int nb)
        => new(rect.Location + new Point(nb), rect.Size);

    /// <summary>
    /// Calls the <see cref="PointExtensions.Modulo(Point, int)"/> function on the rectangle's location. The size remains unchanged.
    /// </summary>
    /// <returns>The new rectangle</returns>
    public static Rectangle Modulo(this Rectangle rect, int nb)
        => new(rect.Location.Modulo(nb), rect.Size);
    ...
    /// </summary>
    /// <returns></returns>
    public static Rectangle Modulo(this Rectangle rect, Point point)
        => new(rect.Location.Modulo(point), rect.Size);

    /// <summary>
    /// Returns the accurate center point of the rectangle
    /// </summary>
    /// <returns>The middle point of the rectangle</returns>
    public static Vector2 AccurateCenter(this Rectangle rect)
        => rect.Location.ToVector2() + rect.Size.ToVector2() * 0.5f;

    /// <summary>
    /// Returns a list of points that contains each point on a Rectangle's perimeter.
    /// </summary>
    /// <returns>A list of points</returns>
    public static IEnumerable<Point> PerimeterPoints(this Rectangle rect)
        => rect.Location.PerimeterPoints(rect.Location + rect.Size.Add(-1));
}
